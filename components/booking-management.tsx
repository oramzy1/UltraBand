...
import { Calendar, Clock, MapPin, Mail, Phone, DollarSign, MessageSquare, Users } from "lucide-react"
...
export function BookingManagement({ bookings, onBookingsUpdate }: BookingManagementProps) {
  ...
  const updateBookingStatus = async (
    ...
    try {
      ...
      // Update local state
      const updatedBookings = bookings.map((booking) => (booking.id === bookingId ? { ...booking, ...data } : booking))
        ...
        {bookings.map((booking) => (
          <Card key={booking.id} className="hover:shadow-md transition-shadow">
            ...
            <CardContent>
              ...
              <div className="flex gap-2 flex-wrap">
                <Dialog>
                  ...
                  <DialogContent className="max-w-2xl">
                    <DialogHeader>
                      <DialogTitle>Manage Booking - {booking.client_name}</DialogTitle>
                    </DialogHeader>
                    <div className="space-y-6">
                      {/* Quick Actions */}
                      <div className="flex gap-2 flex-wrap">
                        <Button
                          size="sm"
                          onClick={() => updateBookingStatus(booking.id, "accepted", adminNotes)}
                          disabled={isUpdating}
                        >
                          Accept
                        </Button>
                        <Button
                          size="sm"
                          variant="destructive"
                          onClick={() => updateBookingStatus(booking.id, "rejected", adminNotes)}
                          disabled={isUpdating}
                        >
                          Reject
                        </Button>
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => updateBookingStatus(booking.id, "pending", adminNotes)}
                          disabled={isUpdating}
                        >
                          Mark Pending
                        </Button>
                      ...
                      <div className="space-y-4">
                        ...
                        <Button
                          onClick={() =>
                            updateBookingStatus(booking.id, "counter_proposed", adminNotes, proposedDate, proposedTime)
                          }
                          disabled={isUpdating || !proposedDate || !proposedTime}
                        >
                ...
                {booking.status === "pending" && (
                  <>
                    <Button size="sm" onClick={() => updateBookingStatus(booking.id, "accepted")} disabled={isUpdating}>
                      Quick Accept
                    </Button>
                    <Button
                      size="sm"
              ...
              {booking.status === "counter_proposed" && booking.proposed_date && booking.proposed_time && (
                <div className="mt-4 p-3 bg-blue-500/10 border border-blue-500/20 rounded-md">
                  <p className="text-sm font-medium text-blue-600 mb-1">Counter Proposal Sent</p>
                  <p className="text-sm text-blue-600">
                    Proposed: {format(new Date(booking.proposed_date), "PPP")} at {booking.proposed_time}
                  </p>
                </div>
              )}
            </CardContent>
