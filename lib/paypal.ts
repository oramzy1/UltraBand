...
export async function generatePayPalLink(data: {
    ...
    try {
      // Call the function below to get access token
      const accessToken = await getPayPalAccessToken();
    ...
    const apiUrl = 'https://api-m.sandbox.paypal.com/v2/checkout/orders'
      ...
      const response = await fetch(apiUrl, {
        ...
        body: JSON.stringify({
          ...
          application_context: {
            brand_name: 'Ultra Band Music',
            shipping_preference: 'NO_SHIPPING', // Add this line
            user_action: 'PAY_NOW', // Add this line
            return_url: `${process.env.NEXT_PUBLIC_APP_URL}/payment/success?booking_id=${data.bookingId}`,
            cancel_url: `${process.env.NEXT_PUBLIC_APP_URL}/payment/cancel?booking_id=${data.bookingId}`,
          },
        }),
      });
  ...
  // This function was already created earlier
  async function getPayPalAccessToken() {
    try {
      const auth = Buffer.from(
        `${process.env.PAYPAL_CLIENT_ID}:${process.env.PAYPAL_CLIENT_SECRET}`
